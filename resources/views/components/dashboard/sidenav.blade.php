<aside class="min-h-screen col-span-1 px-8 bg-green-200 shadow">
        ...
        <div>
            ...
            <div>
                <x-sidenav.link href="{{ route('profile.show') }}" :active="request()->routeIs('profile.show')">
                    <x-zondicon-user class="w-3" style="color: #283044;" />
                    <span style="color: #283044;">{{ __('Profile') }}</span>
                </x-sidenav.link>
            </div>
            ...
            <div>
                <x-sidenav.link href="{{ route('dashboard.notifications.index') }}" :active="request()->routeIs('dashboard.notifications.index')">
                    <x-zondicon-notifications-outline class="w-3" style="color: #283044;" />
                    <span style="color: #283044;">{{ __('Notifications') }}</span>
                </x-sidenav.link>
            </div>
        </div>
        ...
        <div>
            ...
            <div>
                <x-sidenav.link href="{{ route('admin.categories.index') }}" :active="request()->routeIs('admin.categories.index')">
                    <x-zondicon-view-tile class="w-3" style="color: #283044;" />
                    <span style="color: #283044;">{{ __('Index') }}</span>
                </x-sidenav.link>
            </div>
            <div>
                <x-sidenav.link href="{{ route('admin.categories.create') }}" :active="request()->routeIs('admin.categories.create')">
                    <x-zondicon-compose class="w-3" style="color: #283044;" />
                    <span style="color: #283044;">{{ __('Create') }}</span>
                </x-sidenav.link>
            </div>
        </div>
        @endif
            ...
            <div>
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <x-sidenav.link href="{{ route('logout') }}" onclick="event.preventDefault();                                               this.closest('form').submit();">
                        <x-heroicon-o-logout class="w-4" style="color: #283044;" />
                        <span style="color: #283044;">{{ __('Logout') }}</span>
                    </x-sidenav.link>
